...
struct svm_model
{
  ...
  // --
  // NP: libsvm varma
  double* obj; // svm objectives = varma cost
  // --
...
// --
// NP: libsvm varma
double svm_varma_get_obj(const svm_model *model, const int label)
{
	return model->obj[label];
}
double svm_varma_get_rho(const svm_model *model, const int label)
{
	return model->rho[label];
}
int svm_varma_get_nSV(const svm_model *model, const int label)
{
	return model->nSV[label];
}
int svm_varma_get_total_sv(const svm_model *model)
{
  return model->l;
}
void svm_varma_get_sv_coef(const struct svm_model *model, const int label, double* sv_coef)
{
  for(int i=0; i<model->l; ++i)
    {
      sv_coef[i] = model->sv_coef[label][i];
    }
}
void svm_varma_get_SV(const struct svm_model *model, double* SV)
{
  int total_sv = model->l;
  
  for(int i=0; i<total_sv; ++i)
    {
      SV[i] = model->SV[i][0].value;
    }
}
// --
...
svm_model *svm_load_model(const char *model_file_name)
{
	...
	// --
	// NP: libsvm varma (not yet supported here!)
	model->obj = NULL;
	// --
